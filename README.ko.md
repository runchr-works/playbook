...
사용하는 MCP 클라이언트에 Intentir를 추가합니다. 에이전트에게 요청하거나 직접 설정 파일을
만들 수 있습니다.
...
전역 설치 없이 시험하려면 다음과 같이 실행할 수 있습니다.
...
초기화해야 합니다. 이 명령은 `.intentir/config.json`에 bank ID를 저장하고 로컬 CodeGraph 인덱스를
초기화합니다.
...
그다음 Pi에게 Intentir를 추가해달라고 요청합니다.
...
Pi가 프로젝트 루트에 `.mcp.json`을 생성합니다. 직접 만들 수도 있습니다:
