...
export function agentsCommand(action: string | undefined, args: string[]): void {
  ...
  if (action === "config") {
    ...
    if (!requested) throw new Error("Usage: intentir agents config <agent> [--root <path>]");
    const agent = findAgent(requested);
    if (!agent) throw new Error(`Unsupported agent: ${requested}. Run \`intentir agents list\`.`);
