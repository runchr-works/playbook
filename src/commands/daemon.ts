...
function pidPath(env: NodeJS.ProcessEnv = process.env): string {
  return path.join(intentirHome(env), "hindsight.pid");
}

export function hindsightPid(env: NodeJS.ProcessEnv = process.env): number | undefined {
...
export async function startHindsightDaemon(
  hindsightEnv: NodeJS.ProcessEnv,
  ...
  try {
    ...
    await log.close();
    throw new Error(
      `Failed to start Hindsight. Check that \`uvx\` is available: ${
...
export function stopHindsightDaemon(env: NodeJS.ProcessEnv = process.env): boolean {
...
export async function runHindsightDaemon(
  hindsightEnv: NodeJS.ProcessEnv,
  processEnv: NodeJS.ProcessEnv = process.env,
): Promise<number> {
  const existing = hindsightPid(processEnv);
...
export async function daemonCommand(
  ...
  if (!config) throw new Error("Intentir is not onboarded. Run `intentir onboard` first.");
  ...
  if (action === "start") {
    ...
    const ready = wait
      ? await waitForHindsight(
          baseUrl,
          Number(env.INTENTIR_DAEMON_START_TIMEOUT_MS ?? 180_000),
        )
      : (await checkHindsightHealth(baseUrl)).ok;
    console.log(JSON.stringify({ started: true, pid, ready, baseUrl }, null, 2));
    if (wait && !ready) process.exitCode = 1;
    return;
  }
