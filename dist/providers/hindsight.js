...
export class HindsightProvider {
    ...
    async recall(input) {
        const scopes = ["agent-private", "project-shared"];
        ...
        if (results.length === 0 && settled.every((result) => result.status === "rejected")) {
            throw new AggregateError(settled.map((result) => result.status === "rejected" ? result.reason : undefined), "All Hindsight recalls failed");
        }
        const seen = new Set();
    ...
    async retain(input, scope = "agent-private") {
        const sourceId = randomUUID();
        return this.write(input, scope, sourceId);
    }
    promote(input, sourceId, method = "automatic") {
        return this.write({
            ...input,
            context: `intentir:${method}-promoted;source=${sourceId}`,
        }, "project-shared", sourceId);
    }
    async review(input) {
    ...
    async write(input, scope, sourceId) {
        const bank = bankId(input.identity, scope);
