...
import { AGENTS, findAgent, type AgentDescriptor } from "./agents.js";
...
function readJson(filename: string): JsonObject {
  ...
  return JSON.parse(readFileSync(filename, "utf8")) as JsonObject;
}
...
function standardServers(connections: McpConnections): JsonObject {
...
function mergeStandardJson(filename: string, connections: McpConnections): void {
  const doc = readJson(filename);
  const servers = (doc.mcpServers as JsonObject | undefined) ?? {};
  Object.assign(servers, standardServers(connections));
  doc.mcpServers = servers;
  writeJson(filename, doc);
}

function mergeOpenCodeJson(filename: string, connections: McpConnections): void {
  const doc = readJson(filename);
  const servers = (doc.mcp as JsonObject | undefined) ?? {};
  servers.codegraph = {
    type: "local",
...
export function renderCodexConfig(connections: McpConnections): string {
  return renderCodexConfigBlock(connections, true, true);
}

function renderCodexConfigBlock(
  connections: McpConnections,
  includeHindsight: boolean,
  ...
  if (connections.hindsightUrl && includeHindsight) {
    lines.push(
      ...
      `url = ${tomlString(connections.hindsightUrl)}`,
      "",
    );
  }
  if (includeCodegraph) {
    lines.push(
      ...
      `command = ${tomlString(connections.codegraphCommand)}`,
      `args = ${JSON.stringify(connections.codegraphArgs)}`,
    );
  }
...
function mergeCodexToml(filename: string, connections: McpConnections): void {
  ...
  const block = renderCodexConfigBlock(
    connections,
    !/^\s*\[mcp_servers\.hindsight\]\s*$/m.test(unmanaged),
    !/^\s*\[mcp_servers\.codegraph\]\s*$/m.test(unmanaged),
  );
  let next: string;
...
function mergeTextBlock(filename: string, startMarker: string, endMarker: string, block: string): void {
  const current = existsSync(filename) ? readFileSync(filename, "utf8") : "";
...
export function configureGlobalContextMode(
  agentId: string,
  home = os.homedir(),
): AgentConfigResult {
  try {
    if (agentId === "codex") {
      ...
      const current = existsSync(filename) ? readFileSync(filename, "utf8") : "";
      if (
        !current.includes(CODEX_CONTEXT_START) &&
...
export function projectConfigPath(agentId: string, repositoryRoot: string): string | undefined {
  switch (agentId) {
    case "codex":
...
export function renderAgentConfig(
  ...
  if (agent.id === "codex") return renderCodexConfig(connections);
  ...
  if (!filename) return `Manual setup required: ${agent.configLocation}\n`;
  return `${JSON.stringify({ mcpServers: standardServers(connections) }, null, 2)}\n`;
}

function mergeOpenCodeJsonObject(doc: JsonObject, connections: McpConnections): void {
  const servers: JsonObject = {
    codegraph: {
      type: "local",
      command: [connections.codegraphCommand, ...connections.codegraphArgs],
...
export function normalizeAgentIds(values: string[]): string[] {
  const ids = values
...
export function supportedAutomaticAgents(): AgentDescriptor[] {
