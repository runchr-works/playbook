...
function parse(args: string[]): ParsedOptions {
...
export async function workspaceCommand(action: string | undefined, args: string[]): Promise<void> {
  ...
  if (action === "init") {
    ...
    if (result.code !== 0) {
      throw new Error(
        `memkit bank '${bankId}' was configured, but CodeGraph init failed with exit code ${result.code}`,
      );
    }
    ...
    const agentConfigs = agents.map((agentId) =>
      configureProjectAgent(agentId, input.path, {
  ...
  if (action === "status") {
    ...
    let codegraph: unknown = { ok: false, detail: "workspace_not_initialized" };
    if (state.initialized) {
      const result = await runCommand(
        process.env.CODEGRAPH_COMMAND ?? "codegraph",
        ["status", input.path, "--json"],
        { cwd: input.path },
      );
  ...
  if (action === "sync") {
    ...
    const result = await runCommand(
      process.env.CODEGRAPH_COMMAND ?? "codegraph",
      ["sync", input.path],
      { cwd: input.path, inherit: true },
    );
    if (result.code !== 0) throw new Error(`CodeGraph sync failed with exit code ${result.code}`);
    return;
  }
