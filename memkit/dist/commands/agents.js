...
import { configureProjectAgent, configureGlobalContextMode, normalizeAgentIds, renderAgentConfig, } from "../agent-config.js";
...
export function agentsCommand(action, args) {
    ...
    if (action === "select") {
        ...
        if (!config)
            throw new Error("Run `memkit onboard` before selecting agents");
    ...
    if (action === "apply") {
        ...
        const config = loadUserConfig();
        if (!config)
            throw new Error("Run `memkit onboard` before applying agent configuration");
        const baseUrl = config.env.HINDSIGHT_BASE_URL;
        const connections = {
            ...(baseUrl
                ? {
                    hindsightUrl: `${baseUrl.replace(/\/$/, "")}/mcp/${encodeURIComponent(workspace.config.bankId)}/`,
                }
                : {}),
        ...
        const results = (config.agents ?? []).map((agentId) => configureProjectAgent(agentId, repositoryRoot, connections));
